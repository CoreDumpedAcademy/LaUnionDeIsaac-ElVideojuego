...
public class AngelKnight : MonoBehaviour
{

    public float speed;
    public float health;

    private float knightCooldown;
    private float knightDamage;
    private float knightDashTime;
    public float startKnightDashTime;
    public float startKnightCooldown;

    private Transform target;
    public float knightChaseRange;
    ...
    void Update()
    {
        knightDamage = Stats.skeletonDamage;
        ...
        if (distance < knightChaseRange)
        {
            if (isDashing == false)
            {

                knightCooldown = knightCooldown - Time.deltaTime;
            ...
            else
            {
                ...
                {
                    isDashing = false;
                    knightCooldown = startKnightCooldown;
                }
                else
                {
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.tag == "Player")
        {
            ...
            {
                Player.playerHealth = Player.playerHealth - knightDamage;
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawWireSphere(transform.position, knightChaseRange);
